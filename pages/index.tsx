...
const style = {
  ...
  loginContainer:
    "w-full h-full flex flex-col justify-center items-center pb-48",
  walletConnectButton:
    "text-2xl text-black bg-white font-bold mb-[-3rem] mt-[3rem] px-6 py-4 rounded-full cursor-pointer hover:bg-[#d7dbdc]",
...
const Home = () => {
  ...
  const userLoggedIn = (
    <div className={style.content}>
      <Sidebar initalSelectedIcon={"Home"} />
      <Feed />
      <Widgets />
    </div>
  );
  ...
  const error = (
    <div className={style.loginContainer}>
      <Image src={errorImg} width={250} height={200} alt="metamask logo" />
      <div className={style.loginContent}>An error occurred.</div>
    </div>
  );
